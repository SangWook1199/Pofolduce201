...
document.addEventListener("click", (e) => {
	...
	//회사 인증 업로드
	else if (modalTitle === "회사 인증하기") {

		// alert 띄우기
		alert("회사 인증 정보를 전송했습니다.");
...
document.addEventListener("DOMContentLoaded", () => {
	...
	editBtn.addEventListener("click", (e) => {
		...
		if (isDisabled) {
			...
			if (!companyName || !verification) return;

			// 회사 이름이 보이는 경우만 수정 모드로 전환
			if (companyName.style.display !== "none") {
				companyName.style.display = "none";
				verification.style.display = "inline-block"; // 인증 버튼 표시
			}
			// 이미 companyCertification이 'no'이면 버튼은 이미 보이므로 변경 없음
		} else {
			// 수정 완료: AJAX로 전송
